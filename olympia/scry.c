...
scry_show_where(int who, int target)
{
	...
	{
		out(who, "%s is in %s.", box_name(target),
				just_name(region(target)));
		return;
	}

	out(who, "%s is in %s.", box_name(target),
			box_name(province(target)));
}
...
d_shroud_region(struct command *c)
{
	...
	wout(c->who, "%s is now cloaked with an aura %s location shroud.",
		...
		loop_char_here(where, n)
		{
			...
			if (has_skill(n, sk_shroud_region))
			{
				wout(n, "%s cast %s here.  %s is now cloaked "
					"with an aura %s location shroud.",
						box_name(c->who),
...
v_bar_loc(struct command *c)
{
	...
	{
		wout(c->who, "%s already has a permanent barrier.",
					box_name(where));
		return FALSE;
	}
...
d_bar_loc(struct command *c)
{
	...
	if (p->barrier > 0)
		wout(c->who, "The barrier has aura %s.", nice_num(p->barrier));
	else
		wout(c->who, "The barrier is permanent.");

	if (old_val == 0)
	{
		out(where, "%s cast a magical barrier over %s.",
				box_name(c->who), box_name(where));
	}
...
d_unbar_loc(struct command *c)
{
	...
	{
		old_val = abs(old_val);
		if (kind(old_val) == T_char)
			wout(old_val, "%s removed the barrier over %s.",
					box_name(c->who), box_name(where));
	}
