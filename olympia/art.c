...
destroy_item(struct command *c, int item)
{
	...
	if (ret)
	{
		...
		if (aura > 0)
		{
			...
			wout(c->who, "Gained %s current aura.",
						nice_num(aura));
		}
...
v_show_art_creat(struct command *c)
{
	...
	if (has_item(c->who, item) < 1)
	{
		wout(c->who, "%s has no %s.", box_name(c->who),
						box_code(item));
		return FALSE;
	}
 ...
 */
	...
	if (!destroyable_item(item))
	{
		wout(c->who, "The curse can not be applied to %s.",
					box_name(item));
		return FALSE;
	}
...
int
v_use_ancient_aura(struct command *c)
{
	int item = c->a;
	int target = c->b;

	if (kind(target) != T_item || subkind(target) != sub_dead_body)
	{
		wout(c->who, "%s isn't a dead body.", box_name(target));
		return FALSE;
	}

	if (has_item(c->who, target) < 1)
	{
		wout(c->who, "Don't have %s.", c->parse[2]);
		return FALSE;
	}
...
int
d_use_ancient_aura(struct command *c)
{
	int item = c->a;
	int target = c->b;
	...
	wout(c->who, "%s vanishes.", box_name(target));
	dead_body_np = FALSE;
	destroy_unique_item(c->who, target);
	dead_body_np = TRUE;
