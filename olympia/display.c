...
liner_desc_ship(int n)
{
	...
	if (fee = board_fee(n))
		/* BUGFIX (modernization):
		 * The original source passed three arguments to strcat(), which is invalid C.
		 * All other call sites use sout() to format a temporary string before appending.
		 * This change restores the intended behavior without altering program logic.
		 *
		 * NOTE: sout() allocates memory. This leak is intentional and consistent with
		 * Olympia’s one-cycle execution model (“run once and exit”).
		 */
...
static void
show_weather(int who, int where)
{
	int rain, wind, fog;
