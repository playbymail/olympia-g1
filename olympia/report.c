...
show_char_inventory(int who, int num)
{
	...
	if (item_ents_len(bx[num]->items) > 0)
	{
		qsort(bx[num]->items, item_ents_len(bx[num]->items),
					sizeof(struct item_ent *), inv_item_comp);
	}
...
player_banner(void)
{
	...
	loop_player(pl)
	{
		...
		out(pl, "Subject: Olympia g1 turn %d report", sysclock.turn);
		out(pl, "");
#endif
		...
		case 1:
			indent += 3;
			wout(pl, "Note:  Please download the Olympia Times "
		...
		case 2:
			...
			wout(pl, "Note:  Olympia Times mailing is turned off for you."
				"To begin receiving the Times again, issue the order TIMES 0");
			indent -= 3;
			out(pl, "");
			break;
		}
		...
		wout(pl, "Olympia g1 turn %d report for %s.",
...
report_account_sup(int pl)
{
	...
	sprintf(cmd, "/u/oly/bin/acct -g g1 -p %s -s 4 > %s",
...
charge_account(void)
{
	...
	loop_player(pl)
	{
		...
		sprintf(cmd, "/u/oly/bin/acct -g g1 -p %s -t%s -y \"olympia g1 turn %d\"",
				box_code_less(pl),
				val_s,
				sysclock.turn);
