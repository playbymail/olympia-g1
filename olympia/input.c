...
#include	<stdio.h>
#include "legacy.h" /* BUGFIX (modernization): forward declarations */
#include	"z.h"
#include	"oly.h"
...
/*
 *  Line parser.  Given a list of char *'s and a line,
 *  returns the list with pointers into the cut-up line.
 *
 *  Depends on pointer-to-int conversions being safe
 */

cstrings_list
parse_line(cstrings_list l, char *s)
{
	char *p;
	char *prev;
 ...
 */
	...
	if (n == 0 && who && subloc(who) && fuzzy_strcmp(s, "garrison"))
	{
...
sort_load_queue(ilist l)
{
	...
	qsort(l, (size_t) ilist_len(l), sizeof(int), exec_comp);
}
...
interrupt_order(int who)
{
	...
	if (stack_leader(who) == who)
		restore_stack_actions(who);	/* not moving anymore */
