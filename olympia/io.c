...
scan_loc(struct entity_loc *p, int box_num)
{
	...
	while (line && iswhite(*line))
	{
		...
		case 'ds':	p->dist_from_sea = (schar)(atoi(t));	break;
		case 'dw':	p->dist_from_swamp = (schar)(atoi(t));	break;
		case 'dg':	p->dist_from_gate = (schar)(atoi(t));	break;
		case 'ng':	p->near_grave = box_scan(t);	break;
...
scan_player(struct entity_player *p, int box_num)
{
	...
	while (line && iswhite(*line))
	{
		...
		case 'ti':	p->no_times_credit = (schar)(atoi(t));	break;
...
load_system(void)
{
	...
	while (s = getlin_ew(fp))
	{
		...
		{
			seed[0] = (unsigned short)(atoi(&s[6]));
		}
		else if (strncmp(s, "seed1=", 6) == 0)
		{
			seed[1] = (unsigned short)(atoi(&s[6]));
		}
		else if (strncmp(s, "seed2=", 6) == 0)
		{
			seed[2] = (unsigned short)(atoi(&s[6]));
		}
 ...
 */
...

	{
		...
		fprintf(stderr, "%d boxes, %d used (%d%%), %d free (%d%%)\n",
				MAX_BOXES,
				used, used * 100 / MAX_BOXES,
				unused, unused * 100 / MAX_BOXES);
	}
}
