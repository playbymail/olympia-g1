#include <stdarg.h> /* BUGFIX (modernization): use varargs */
#include	<stdio.h>
#include	<limits.h>
#include <stdlib.h>
...
int init_spaces(void)
{
	...
	spaces_len = 150;
	spaces = my_malloc((unsigned) (spaces_len + 1));
...
sout(const char *format, ...)
{
	...
	vsnprintf(buf, BUF_LEN + 1, format, args);
	va_end(args);
...
can_view_loc(int pl, struct entity_player *p, int where, int outer)
{
	...
	if (show_to_garrison && ilist_lookup(l, pl) >= 0)
	{
...
out_garrison(int garr, char *s)
{
	...
	l = players_who_rule_here(garr);

	for (i = 0; i < ilist_len(l); i++)
	{
		if (l[i] && subkind(l[i]) != sub_pl_silent)
			bottom_out(l[i], OUT_GARR, garr, s);
	}
}
...
void
wout(int who, const char *format, ...)
{
	char buf[LEN];
	va_list args;
	va_start(args, format);
...
void
wiout(int who, int ind, const char *format, ...)
{
	char buf[LEN];
...
log(int k, const char *format, ...)
{
	...
	// BUGFIX (modernization): can't forward varadic arguments */
