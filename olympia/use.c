...
/* command handlers are declared in proto.h (via oly.h) */
...
{"c", sk_mage_menial,    v_mage_menial,    d_mage_menial,    NULL,     7, 0},
...
fractional_skill_qualifier(struct skill_ent *p)
{
	...
	if (frac == 0)
		return sout("%d/%d", p->days_studied / TOUGH_NUM,
				learn_time(p->skill));

	return sout("%d.%d/%d", p->days_studied / TOUGH_NUM,
				frac * 100 / TOUGH_NUM,
				learn_time(p->skill));
}
...
v_study(struct command *c)
{
	...
	{
		wout(c->who, "Instruction in %s is not available here.",
						box_code(sk));
		return FALSE;
	}
...
d_study(struct command *c)
{
	...
	p->days_studied += TOUGH_NUM * diminish / 7;

	if (p->days_studied >= learn_time(sk) * TOUGH_NUM)
	{
 ...
 */
	...
	wout(c->who, "Research uncovers a new skill:  %s",
					box_name(new_skill));
