...
static void
show_post(cstrings_list l, int cmd)
{
	int i;
	char attrib[100];
	char *t;
...
eat_banner()
{
	...
	out(eat_pl, "X-Loop: orders@g1.pbm.com");
	out(eat_pl, "Bcc: g1watch");
	out(eat_pl, "");
...
#if 0
		ret = system(sout("rep %s/log/%d | %srmail %s g1watch",
				libdir, eat_pl, entab(eat_pl), who_to));
#else
		ret = system(sout("rep %s/log/%d | %ssendmail -t",
