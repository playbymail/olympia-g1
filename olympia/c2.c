 ...
 */
	...
	if (save_flag)
	{
		sprintf(cmd, "/u/oly/bin/acct -p %s -g g1 -d", box_code_less(pl));
		system(cmd);
	}
...
void
text_list_free(cstrings_list l)
{
	int i;
...
static int
line_length_check(cstrings_list l)
{
	int i;
	int len = 0;
	...
	if (lines == 0)
	{
		while (!done)
		{
			...
			if (i_strcmp(t, "end") == 0)
				...
				cstrings_append(&l, str_save(order));
			pop_order(pl, c->who);
		}
	}
	else
...
times_masthead()
{
	...
	sprintf(issue_s, "issue g1-%d", sysclock.turn);

	fprintf(fp, "From: olympia@pbm.com (Olympia PBEM)\n");
	fprintf(fp, "Subject: Olympia Times g1-%d\n", sysclock.turn);
	fprintf(fp, "To: olympia@pbm.com\n");
	fprintf(fp, "Bcc: totimes-g1\n\n");
	fprintf(fp, "   +----------------------------------------------------------------------+\n");
...
times_credit(struct command *c)
{
	...
	{
		wout(c->who, "%s not a character.", c->parse[2]);
		return FALSE;
	}

	pl = player(target);

	if (no_times_credit(pl))
	{
		wout(c->who, "The Times refuses to pay you.");
		return FALSE;
	}
...
v_fee(struct command *c)
{
	...
	wout(c->who, "Ship boarding fee set to %s per 100 weight.",
					gold_s(amount));
 ...
 */
	...
	{
		wout(c->who, "Can't afford a boarding fee of %s.",
				gold_s(amount));
