...
/*
 *  Forward declarations for structs defined privately inside a single .c
 *  file (build.c, produce.c, make.c, combat.c, npc.c).  Declaring the tags
 *  at file scope here keeps the prototypes below referring to the real
 *  file-scope struct rather than a distinct prototype-scope tag.
 */
struct make;
struct harvest;
struct build_ent;
...
extern int d_use_ancient_aura(struct command *c);
...
extern int v_use_ancient_aura(struct command *c);
...
extern int d_mage_menial(struct command *c);
...
/* olympia/build.c */
extern int build_structure(struct command *c, struct build_ent *bi);
...
extern int start_build(struct command *c, struct build_ent *bi);
...
extern int v_tell(struct command *c);
...
extern int d_exhume(struct command *c);
...
extern int v_exhume(struct command *c);
...
/* olympia/quest.c */
extern int seed_subloc_with_monster(int where);
extern void seed_monster_sublocs(void);
