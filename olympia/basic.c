...
v_meditate(struct command *c)
{
	...
	if (char_cur_aura(c->who) >= max_eff_aura(c->who) + 1)
	{
...
v_adv_med(struct command *c)
{
	...
	if (char_cur_aura(c->who) >= max_eff_aura(c->who) + 2)
	{
		wout(c->who, "Current aura is already %d.  It may not "
			"be increased further via advanced meditation.",
					char_cur_aura(c->who));
		return FALSE;
	}
...
v_mage_menial(struct command *c)
{
	...
	if (rp_magic(c->who) && rp_magic(c->who)->mage_worked)
	{
	    wout(c->who, "May only engage in menial tasks once per month.");
	    return FALSE;
	}

	if (loc_depth(where) != LOC_province && subkind(where) != sub_city)
	{
		wout(c->who, "Must be in a city or a province.");
		return FALSE;
	}
 ...
 */
	...
	if (!consume_item(where, item_mage_menial, 1))
	{
		wout(c->who, "A magician has already worked here this month.");
		return FALSE;
	}
...
d_mage_menial(struct command *c)
{
	...
	p_magic(c->who)->mage_worked = TRUE;
	gen_item(c->who, item_gold, 100);
	gold_common_magic += 100;

	wout(c->who, "Earned %s%s.", gold_s(100), mage_menial_how());
	show_to_garrison = TRUE;
	wout(where, "%s earned %s working at common magic.",
					box_name(c->who), gold_s(100));
