 ...
 * This file implements `sub_item` as static, so it must have a forward declaration.
 */
 ...
 */
...

	{
		...
		out(pl, "%s was buried in %s, in %s.",
				box_name(who),
				box_name(grave),
				box_name(province(grave)));
	...
	if (subkind(item) == sub_dead_body)
	{
		...
		if (valid_box(pl) && dead_body_np)
		{
			...
			out(pl, "%s~%s has passed on.  Gained 1 NP.",
				rp_misc(item)->save_name, box_code(item));
		}
