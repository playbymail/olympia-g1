...
d_detect_gates(struct command *c)
{
	...
	if (ret == FALSE)
		ret = list_province_gates(c->who, province(c->who));

	if (ret == FALSE)
		list_nearby_gates(c->who, province(c->who));
...
v_reveal_key(struct command *c)
{
	...
	wout(c->who, "Attempt to learn the key for %s.", box_name(gate));
	return TRUE;
}
...
v_seal_gate(struct command *c)
{
	...
	if (sealed != 0)
	{
		wout(c->who, "%s is already sealed.", box_name(gate));
		return FALSE;
	}
