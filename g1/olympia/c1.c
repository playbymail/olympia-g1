 ...
 */
		if (subkind(where) == sub_city &&
		    subkind(e->item) == sub_suffuse_ring)
			continue;
...
v_times(struct command *c)
{
	...
	if (p->compuserve)
		...
		wout(c->who, "Will receive the Times.");
...
v_add_ram(struct command *c)
{
	...
	{
		wout(c->who, "The galley must be completed before a "
						"ram may be added.");
		return FALSE;
	}

	if (subkind(ship) != sub_galley)
	{
		wout(c->who, "Must be inside a galley to add a ram.");
		return FALSE;
	}
...
parse_wait_args(struct command *c)
{
	...
	while (i <= numargs(c))
	{
		...
		switch (tag)
		{
		case 16:	tag = 11;	break;
		case 17:	tag = 12;	break;
		case 18:	tag = 13;	break;
		}
