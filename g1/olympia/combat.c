...
fort_covers(int n)
{
	switch (subkind(n))
	{
	case sub_castle:		return 500;
	case sub_castle_notdone:	return 500;
	case sub_tower:			return 100;
	case sub_tower_notdone:		return 100;
	case sub_galley:		return 50;
	case sub_galley_notdone:	return 50;
	case sub_roundship:		return 50;
	case sub_roundship_notdone:	return 50;
	case sub_temple:		return 50;
	case sub_temple_notdone:	return 50;
	case sub_inn:			return 50;
	case sub_inn_notdone:		return 50;
	case sub_mine:			return 50;
	case sub_mine_notdone:		return 50;
...
static int
siege_engine_useful(fights_list l)
{
...
static int
who_protects(fights_list l, int pos)
{
	int i = pos;
...
static void
init_attack_defense(fights_list l)
{
	int i;
	struct fight *f;
	int mk;
 ...
 */
		...
		default:
			...
			if (combat_rain && (f->kind == item_archer ||
...
static void
add_to_fight_list(fights_list *l, int unit, int kind, int num,
						int ally, int inside)
{
	struct fight *new;
...
static void
add_fighters(fights_list *l, int who, int ally, int inside)
{
	struct item_ent *e;
...
static void
add_fight_stack(fights_list *l, int who, int ally)
{
	int i;
	int inside = FALSE;
	...
	if (fights_len(*l) > 0 && (*l)[0]->kind == FK_fort &&
	    somewhere_inside((*l)[0]->unit, who))
		inside = TRUE;

	add_fighters(l, who, ally, inside);

	loop_char_here(who, i)
	{
		add_fighters(l, i, ally, inside);
	}
	next_char_here;
}
...
static void
look_for_allies(fights_list *l, int where, int def1, int def2, int attacker)
{
	int i;
 ...
 */
		...
		/* BUGFIX (modernization): yes, they do store ints and pointers in the same list */
		if (fights_lookup(l_a, (struct fight *)((long)i)) >= 0)
			continue;

		add_fight_stack(&l, i, FALSE);
	}
	next_here;
...
static void
ready_fight_list(fights_list l)
{
...
static void
reclaim_fight_list(fights_list *l)
{
	int i;
...
static int
num_attackers(struct fight *f, fights_list enemy)
{
...
static int
total_attackers(fights_list l, fights_list enemy)
{
	int i;
	int sum = 0;
...
static int
num_targets(struct fight *f, fights_list enemy)
{
...
static int
num_valid_targets(struct fight *f, fights_list enemy)
{
...
static int
total_valid_targets(fights_list l, fights_list enemy)
{
	int i;
	int sum = 0;
	int count = 0;
...
static int
total_non_damage(fights_list l)
{
	int i;
	int sum = 0;
...
static int
total_combat_sum(fights_list l)
{
	int i;
	int sum = 0;
...
static void
decrement_num(fights_list l, struct fight *attacker, struct fight *g)
{
	int hit;
 ...
 */
	...
	if (g->num <= 0 && g->protects >= 0)
	{
		...
		out(combat_pl, "    %s.%d no longer protects %s",
				...
				g->kind,
				box_code_less(l[g->protects]->unit));
...
static void
resolve_hit(fights_list l, struct fight *f, struct fight *g, int man)
{
	int n;
...
static struct fight *
find_attacker(fights_list l, int man, fights_list enemy)
{
	int i;
...
static struct fight *
find_defender(fights_list l, int man, fights_list enemy)
{
	int i;
...
static void
choose_attack(int attacker, fights_list l_a, fights_list l_b)
{
	int num_defend;
	struct fight *f;
	struct fight *g;
...
static void
combat_round(fights_list l_a, fights_list l_b, int force_win)
{
	int num_attack_a;
	int num_attack_b;
	int man;
...
/*
 *  loop min(num_attack_a, num_attack_b) times?
 */
...
#if 0
	assert(num_attack_a > 0);
	assert(num_attack_b > 0);
#else
...
static int
side_has_skill(fights_list l, int sk)
{
	int i;
...
static void
deduct_dead(fights_list l_a, fights_list l_b, int inherit)
{
	int i;
	int unit, item;
	int amount;
...
static void
determine_noble_wounds(fights_list l)
{
	int i;
...
static void
check_fatal_survive(fights_list l)
{
	int i;
...
static void
structure_damage(fights_list l)
{
	int unit;
...
static void
determine_prisoners(fights_list l_a, fights_list l_b)
{
	int num_a, num_b;
...
static void
take_prisoners(int winner, fights_list l)
{
	int i;
...
static void
demote_units(int winner, fights_list l)
{
	int i;
...
static char *
combat_display_with(fights_list l)
{
	int i;

	for (i = 1; i < fights_len(l); i++)
		if (l[i]->kind == FK_noble)
		{
			if (l[0]->kind == FK_fort)
				return ", owner:";
...
static void
combat_banner(fights_list l_a, fights_list l_b)
{
	int i;

	assert(fights_len(l_a) > 0);
	assert(fights_len(l_b) > 0);
	...
	for (i = lead_char_pos(l_a)+1; i < fights_len(l_a); i++)
		if (l_a[i]->kind == FK_noble)
			wout(l_a[i]->unit,
...
static char *
tally_side_losses(fights_list l)
{
	int i;
	char *s = NULL;
...
static char *
tally_personal_losses(fights_list l, int pos)
{
	int i;
	char *s = NULL;

	assert(fights_len(l) > pos);
	assert(l[pos]->kind == FK_noble);

	for (i = pos+1; i < fights_len(l) && l[i]->unit == l[pos]->unit; i++)
...
static char *
what_happened_to_noble(fights_list l, int pos)
{
	char *s = NULL;
	...
	{
#if 0
		if (fights_len(l) > pos+1 && l[pos+1]->unit == l[pos]->unit)
		{
...
static void
show_side_results(fights_list l)
{
	int lead;
	char *tally;
	int i;
...
static int
best_here_pos(fights_list l, int where)
{
	int i;
	int best = 99999;
	int n;
...
static void
combat_stop_movement(int who, fights_list l)
{
	ilist tmp;
	int ship;
	int i;
...
static void
reconcile(int winner, fights_list l_a, fights_list l_b)
{
	int loser;
...
static void
increase_defense(fights_list l)
{
	int i;
...
static int
run_combat(fights_list l_a, fights_list l_b, int force_win)
{
	int num_a, num_b;
...
#if 1
	if (l_a[0]->unit == 49699 && l_b[0]->unit == 36016)
	{
		fprintf(stderr, "scrambling random seed\n");
		init_random();
	}
#endif
...
static int
combat_top(fights_list l_a, fights_list l_b, int force_win)
{
	int result;
...
static int
fail_defeat_check(int a, fights_list l_b)
{
	int lead_b = lead_char(l_b);
	int n;
...
static void
set_second_waits(fights_list l, int already_waiting)
{
	int i;
	struct command *c;
...
regular_combat(int a, int b, int seize_slot, int already_waiting)
{
	...
	if (is_loc_or_ship(b) && fights_len(l_b) < 1)
	{
...
v_pillage(struct command *c)
{
	...
	int men = count_stack_figures(c->who);
	int flag = c->a;
	int guard;
	...
	if (men < 10)
	{
		wout(c->who, "At least 10 men are needed to pillage.");
		return FALSE;
	}
...
d_pillage(struct command *c)
{
	...
	if (men < 10)
	{
		wout(c->who, "No longer have 10 men.");
		return FALSE;
	}
