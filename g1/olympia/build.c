...
int
start_build(struct command *c, struct build_ent *bi)
{
	int new;
	int i;
	char *new_name;
 ...
 */
	...
	if (p->effort_given < p->effort_required ||
...
int
build_structure(struct command *c, struct build_ent *bi)
{
	int who = c->who;
