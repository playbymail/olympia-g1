...
v_credit(struct command *c)
{
	...
	wout(target, "The Times pays %s %s.",
			box_name(target), gold_s(25));
...
v_plugh(struct command *c)
{
	...
	if (sysclock.turn != 18)
	{
		wout(c->who, "Only may be used on turn 18.");
		return FALSE;
	}
	...
	default:
		wout(c->who, "You are not on the list of people who "
			"may use this command.");
		return FALSE;
	}

	log(LOG_SPECIAL, "PLUGH %s (%s)", box_name(c->who), box_name(player(c->who)));

	leave_stack(c->who);
	move_stack(c->who, 36132);	/* Imperial City */

	vector_stack(c->who, TRUE);
	wout(VECT, ">poof!<  A cloud of orange smoke appears and "
				"wisks you away...");
	wout(VECT, "Arrival at %s.", box_name(36132));
