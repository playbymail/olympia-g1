...
#define		SUBLOC_OFF	36120	/* where to start numbering sublocs */
#define		REGION_OFF	60000	/* where to start numbering regions */
...
struct tile {
	...
	roads_list roads; /* BUGFIX (modernization): lists */
};
...
tiles_list inside_list[MAX_INSIDE];	/* list of provinces in each region */
...
int imperial_row;	/* location of imperial city */
int imperial_col;
 ...
 */
			...
			case '*':
				...
				fprintf(stderr, "Imperial city found at (%d,%d)\n", row, col);
				imperial_row = row;
				imperial_col = col;
				break;
...
int n;
{
	...
	for (i = 0; i < tiles_len(inside_list[n]); i++)
	{
		count++;
...
int n;
{
	...
	for (i = 0; i < roads_len(subloc[n]->roads); i++)
	{
		count++;
		if (count == 1)
		{
...
/*
 *  If any neighbor is already marked, skip this hole.
 *  Check for NULL first -- neighbors can be NULL at map edges
 *  or adjacent to other holes. (Bug fix: original code crashed
 *  when called for holes at row 0 or col 0.)
 */
	if ((n && n->mark) || (s && s->mark) || (e && e->mark) || (w && w->mark) ||
	    (nw && nw->mark) || (sw && sw->mark) || (ne && ne->mark) || (se && se->mark))
		return FALSE;
/*
...
/*
 *  If any neighbor is already marked, skip this corner.
 *  Check for NULL first -- neighbors can be NULL at map edges.
 *  (Bug fix: original code crashed at row 1 where north neighbor
 *  at row 0 is a map hole.)
 */
	if ((n && n->mark) || (s && s->mark) || (e && e->mark) || (w && w->mark) ||
	    (nw && nw->mark) || (sw && sw->mark) || (ne && ne->mark) || (se && se->mark))
		return FALSE;
...
int i;
{
	...
	sprintf(nprovs, "%d", tiles_len(inside_list[i]));
 ...
 */
	...
	for (row = 0; row < MAX_ROW; row++)
		...
		{
		    tiles_append(&inside_list[map[row][col]->inside],
					map[row][col]);
		}
}
...
int ins;
{
	...
	for (dir = 1; dir < MAX_DIR; dir++)
	{
		...
		if (!p || p->color != map[row][col]->color)
			continue;
...
int col;
{
	...
	for (i = 0; i < roads_len(map[row][col]->roads); i++)
	{
		count++;
		if (count == 1)
		{
...
int dump_roads()
{
	...
	for (row = 0; row < MAX_ROW; row++)
	    for (col = 0; col < MAX_COL; col++)
		if (map[row][col])
		    for (j = 0; j < roads_len(map[row][col]->roads); j++)
		    {
	...
	for (i = 1; i <= top_subloc; i++)
		for (j = 0; j < roads_len(subloc[i]->roads); j++)
		{
			fprintf(road_fp, "%d road 0\n",
...
random_tile_from_each_region()
{
	...
	for (i = 1; i <= inside_top; i++)
	{
		...
		j = rnd(0, tiles_len(inside_list[i])-1);

		tiles_append(&l, inside_list[i][j]);
	}
	...
	for (i = 0; i < tiles_len(l); i++)
	{
		...
		if (q == NULL || q->terrain == terr_ocean)
		{
			fprintf(stderr, "couldn't shift tour (%d,%d)\n",
						l[i]->row, l[i]->col);
			tiles_append(&other, l[i]);
		} else
			tiles_append(&other, q);
	}
...
int key;
{
	...
	gate_counter++;
	ilist_append(&from->gates_num, gate_counter);
...
int gate_continental_tour()
{
	...
	assert(tiles_len(l) == tiles_len(m));

	fprintf(stderr, "\nContinental gate tour:\n");

	for (i = 0; i < tiles_len(l)-1; i++)
	{
...
/*
 *  50% of the regions each get a hidden stone circle
 *  (but we make sure that starting region gets one)
...
int gate_stone_circles()
{
	...
	{
/*
 *  Skip 50% of regions, but not the one containing the imperial city
 */

		if (rnd(0,1) == 0 &&
		    map[imperial_row][imperial_col]->inside != l[i]->inside)
			continue;

		n = create_a_subloc(l[i]->row, l[i]->col, 1, terr_stone_cir);
		tiles_append(&circs, subloc[n]);
...
int mark_bad_locs()
{
	...
	for (i = 1; i <= inside_top; i++)
		if (strcmp(inside_names[i], "Impassable Mountains") == 0)
		{
			for (j = 0; j < tiles_len(inside_list[i]); j++)
...
int not_random_province(row, col)		/* oh, hack upon hack ... */
int *row;
int *col;
{
	int n;
...
int random_province(row, col, terr)
int *row;
int *col;
int terr;
{
...
int make_appropriate_subloc(row, col)
int row;
int col;
{
	int terr;
...
int make_graveyards()
{
	...
	for (i = 1; i <= inside_top; i++)
	{
		...
		n = tiles_len(inside_list[i]);

		if (n < 10)
			continue;

		l = tiles_copy(inside_list[i]);
		tiles_scramble(l);
